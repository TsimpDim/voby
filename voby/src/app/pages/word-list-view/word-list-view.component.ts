...
export class WordListViewComponent {
  ...
  createWord() {
    ...
    dialogRef.componentInstance.relatedWordClicked.subscribe((word: any) =>
      this.selectOrRedirectWord(word),
    );
    ...
    dialogRef.afterClosed().subscribe((data) => {
      ...
      if (data.word.related_words) {
        data.word.related_words.forEach((rw: any) => {
          ...
          if (idx !== -1) {
            this.filteredWords[idx].related_words.push({
              id: data.word.id,
              word: data.word.word,
              set: data.word.sets,
            });
  ...
  deleteSet() {
    ...
    dialogRef.afterClosed().subscribe((res) => {
      if (res && res.confirmed === true) {
        this.voby.deleteSet(this.setId).subscribe({
          next: () => {
  ...
  addTagToSearch(selectedTagId: number) {
    ...
    if (newTag) {
      ...
      const tagIndexToBeRemoved = this.searchTags.findIndex(
        (t) => t.id === newTag.id,
      );
  ...
  selectedWordDeletedOrUnlinked() {
    const idx = this.filteredWords.findIndex(
      (w) => w.id === this.selectedWord?.id,
    );
    if (idx !== -1) {
  ...
  getVClass(classId: number) {
    this.loadingHeader = true;
    this.voby.getClass(classId).subscribe({
      next: (data: any) => {
        this.vclass = data;
        this.search(false);
      },
  ...
  getSet(setId: number) {
    this.loadingHeader = true;
    this.voby.getSet(setId).subscribe({
      next: (data: any) => {
        this.set = data;
  ...
  @HostListener('document:keydown.alt.w', ['$event']) openWordFormAlt(
    event: KeyboardEvent,
  ) {
    if (this.needSet) {
  ...
  @HostListener('document:keydown.meta.w', ['$event']) openWordFormMeta(
    event: KeyboardEvent,
  ) {
    if (this.needSet) {
      event.preventDefault();
  ...
  @HostListener('document:keydown.alt.f', ['$event']) focusSearchAlt(
    event: KeyboardEvent,
  ) {
    if (this.needSet) {
      event.preventDefault();
  ...
  @HostListener('document:keydown.meta.f', ['$event']) focusSearchMeta(
    event: KeyboardEvent,
  ) {
    if (this.needSet) {
      event.preventDefault();
