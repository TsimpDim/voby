...
export class HotkeysService {
  ...
  shortcuts$: BehaviorSubject<Observable<unknown>[]> = new BehaviorSubject<
    Observable<unknown>[]
  >([]);
      ...
      new Observable((observer) => {
        const handler = (e: any) => {
          ...
          if (e.target.nodeName !== 'INPUT') {
            ...
            e.target.value =
              targetValue.substring(0, caretIdx) +
              merged.result +
              targetValue.substring(caretIdx, targetValue.length + 1);
            this.formData.updateFormControlValue({
